...
### Step 10: GitOps (only for config changes)

> ⚠️ **NEVER manually update `newTag` in `kustomization.yaml`** — CI/CD pipeline automatically updates image tags after building. Only commit gitops changes if you modified gateway routes, ConfigMaps, Secrets, or other infrastructure config.
...
# 4. GitOps ONLY if config changed (routes, configmaps, secrets)
#    ⚠️ git pull --rebase origin main BEFORE commit!
#    ⚠️ NEVER update newTag — CI handles image tags automatically
```
...
- [ ] `wire_gen.go` and `*.pb.go` are only updated via code generation, NO manual edits
- [ ] `bin` directories and generated binaries are removed
