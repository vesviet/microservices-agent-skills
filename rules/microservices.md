...
- **Commenting Rules**: Do not use task tags like "[AGENT-XX]", "[EDGE-XX]", "[P0]", or any specific Jira/Issue tracker bracket formats in code comments. Keep code explanation comments to a maximum of 3 lines, focusing only on the business logic.
- **CRITICAL — GitOps First**: When making infrastructure or deployment changes, ALWAYS `git commit && git push` the gitops repo BEFORE running `kubectl apply/patch` on the cluster. The Git repository is the single source of truth. Never apply changes directly to the cluster without committing first — this prevents drift and ensures recoverability.
